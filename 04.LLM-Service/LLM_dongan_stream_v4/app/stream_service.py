...
def _forward_to_outbox(state: _StreamServiceState, src: Path):
    """
    inbox의 파일을 outbox로 이동하되, 세션별 폴더 구조 유지
    예: inbox/session123/000001.jpg -> outbox/session123/000001.jpg
    """
    try:
        # inbox 기준 상대 경로 계산
        ...
        _log("Forwarded to OUTBOX", {"src": str(src), "dst": str(dst)})
        
    except Exception as e:
        _log("Forward to outbox failed", {"src": str(src), "error": str(e)})
