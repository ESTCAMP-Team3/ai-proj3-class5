// static/js/app.js - Socket.io + REST API 폴링 통합 완전판 + 카메라 통합
const socket = io();
...
let cameraSessionId = makeSessionId();
const TARGET_FPS = 24;
...
function makeSessionId() {
    return 'drowsy-' + Math.random().toString(36).slice(2, 10) + '-' + Date.now().toString(36);
}

const updateCameraStatus = (message, type = 'normal') => {
    // 상태 표시 비활성화 - UI에서 상태 텍스트 제거됨
    console.log(`🎥 카메라 상태: ${message} (${type})`);
};

const initCamera = async () => {
    if (!els.cameraVideo || !els.cameraCanvas) {
        console.warn('🎥 카메라 요소를 찾을 수 없음');
        return false;
    }

    try {
        updateCameraStatus('카메라 권한 요청 중...', 'normal');
        ...
        await new Promise((resolve, reject) => {
            ...
            els.cameraVideo.onerror = reject;
...
const stopCamera = () => {
    ...
    updateCameraStatus('카메라 중지됨', 'normal');
    console.log('🎥 카메라 중지됨');
};
...
const uploadFrame = async (blob) => {
    ...
    try {
        ...
        // 주기적으로 연결 상태 업데이트
        if (uploadSequence % 60 === 0) { // 60프레임마다 (약 2.5초)
            updateCameraStatus(`업로드 중... (${uploadSequence})`, 'connected');
        }

        return true;
    } catch (error) {
        console.warn('🎥 업로드 네트워크 오류:', error);
        return false;
    }
};
...
window.addEventListener('load', () => {
    ...
    cameraBtn.style.cssText = 'margin: 2px; padding: 4px 8px; font-size: 10px; background: #2196F3;';
    cameraBtn.onclick = async () => {
        if (cameraActive) {
            ...
            cameraBtn.textContent = success ? '📹✅' : '📹❌';
        }
    };
    testContainer.appendChild(cameraBtn);
...
window.addEventListener('beforeunload', () => {
    ...
    if (audioState.recognition) {
        try { audioState.recognition.stop(); } catch(e) {}
    }
});