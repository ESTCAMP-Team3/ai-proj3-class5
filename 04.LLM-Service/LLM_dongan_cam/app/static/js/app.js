...
// CSS 클래스 예시: bg-normal, bg-warning, bg-monitor, bg-improve, bg-l1, bg-l2, bg-l3
const stageToBgClass = {
  "정상": "bg-normal",
  "의심경고": "bg-warning",
  "집중모니터링": "bg-monitor",
  "개선": "bg-improve",
...
// 마지막으로 적용한 값(불필요한 리렌더 방지)
let lastApplied = { level: null, stage: null };

async function fetchLatestState() {
  try {
    const res = await fetch("/api/state/latest", { credentials: "same-origin" });
    if (!res.ok) return;

    const j = await res.json();
    if (!j.has_state) return;

    const level = parseInt(j.level_code, 10);
    const stage = j.stage;

    // 변화 없으면 스킵
    if (lastApplied.level === level && lastApplied.stage === stage) return;

    // UI 반영
    const bgClass = stageToBgClass[stage] || "bg-normal";
    // 배경 클래스 교체
    els.stageArea.className = `stage-area ${bgClass}`;
    // 라벨/등급 표시
    els.stageLabel.textContent = stage;
    els.dValue.textContent = `D: ${level}`;
    els.centerMessage.textContent = `${stage} 상태입니다.`; // 필요 시 커스터마이즈

    lastApplied = { level, stage };
  } catch (e) {
    console.warn("fetchLatestState error", e);
  }
}
